...
fn expand_factor_for_mpoly(var_num: usize, lhs: Vec<Scalar>, rhs: Vec<Scalar>) -> Vec<Scalar> {
...
mod test {
    ...
    use ff::PrimeField;
    use rayon::scope;
    use crate::utils::{convert_from_binary, convert_to_binary, expand_factor_for_mpoly, expand_factor_for_upoly};
    ...
    fn test_expand_factor_for_mpoly() {
        ...
        let poly_two = vec![Scalar::from_u128(2), Scalar::one().neg(), Scalar::zero(), Scalar::zero()];
        // product: F(x1,x2) = f1 * f2 = 2 - x2 + 2x1 - x1x2
        //         terms: (0,0) = 2, (0,1)=-1, (1,0)=2, (1,1)=-1
        let target = vec![Scalar::from_u128(2), Scalar::one().neg(), Scalar::from_u128(2), Scalar::one().neg()];
